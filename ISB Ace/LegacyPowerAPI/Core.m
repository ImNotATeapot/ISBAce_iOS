...
-(id)init:(NSURL*)url{
    ...
    //nicode to clear cookies every time PSCore is created
    for (NSHTTPCookie *each in _cookieJar.cookies) {
        [_cookieJar deleteCookie:each];
    }
    return self;
}
...
-(NSDictionary*)getAuthData{
    ...
    [dict setObject:pstoken forKey:@"pstoken"];
    [dict setObject:contextData forKey:@"contextData"];
    [dict setValue:[NSNumber numberWithBool:ldap] forKey:@"ldap"];
    //NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys: pstoken,@"pstoken",contextData,@"contextData",ldap,@"ldap", nil];
    return dict;
}
